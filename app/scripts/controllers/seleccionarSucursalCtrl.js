...
var app = angular.module('nextbook20App')
  app.controller('seleccionar_sucursal_Ctrl', function ($scope, $location, $localStorage, establecimientosService, perfilSucursalService) {
    ...
    $scope.Select_Sucursal = function(index) {
      ...
      perfilSucursalService.Get_Img_Perfil().get({sucursal:index.id}).$promise.then(function(data) {
        $localStorage.imgPerfil = data.img;                   
      },function(error){
        $localStorage.imgPerfil="images/users/avatar-001.jpg";
      });
       
      perfilSucursalService.Get_Img_PerfilUsuario().get({sucursal:index.id}).$promise.then(function(data) {
        $localStorage.imgPerfilUsuario = data.img;
      },function(error){
        $localStorage.imgPerfilUsuario="images/users/avatar-001.jpg";
      });
      
      perfilSucursalService.Get_Img_PerfilEmpresa().get({sucursal:index.id}).$promise.then(function(data) {
        $localStorage.imgPerfilEmpresa = data.img;                   
      },function(error){
        $localStorage.imgPerfilEmpresa="images/users/avatar-001.jpg";
      });
      //--------------------cargar imagen Portada-----------
      perfilSucursalService.Get_Img_Portada().get({sucursal:index.id}).$promise.then(function(data) {
        $localStorage.imgPortada = data.img;
      },function(error){
        $localStorage.imgPortada="images/samples/w1.jpg";
      });
      
      perfilSucursalService.Get_Img_PortadaUsuario().get({sucursal:index.id}).$promise.then(function(data) {
        $localStorage.imgPortadaUsuario = data.img;
      },function(error){
        $localStorage.imgPortadaUsuario="images/samples/w1.jpg";
      });
      
      perfilSucursalService.Get_Img_PortadaEmpresa().get({sucursal:index.id}).$promise.then(function(data) {
        $localStorage.imgPortadaEmpresa = data.img;
      },function(error){
        $localStorage.imgPortadaEmpresa="images/samples/w1.jpg";
      });
      // --------------------cargar imagen Logo-----------
      perfilSucursalService.Get_Img_Logo().get({sucursal:index.id}).$promise.then(function(data) {
