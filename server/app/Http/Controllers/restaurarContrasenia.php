...
class restaurarContrasenia extends Controller {
  ...
  public function recuperaClave(Request $request) {
    ...
    $existenciaRucEnSistema = DB::connection('nextbookconex')
      ->table('administracion.empresas')
      ->select('nick')
      ->where('ruc_ci',$request->ruc)->first();
    ...
    if ($existenciaRucEnSistema) {
      if ($request->correo === null && $request->nick !== null) {
        $camposUsuario = ['id','id_persona', 'estado_clave','fecha_actualiza'];

        $idUsuario = DB::connection($name_bdd)
        ->table('usuarios.usuarios')
        ->select($camposUsuario)
        ->where('nick',$request->nick.'@'.config('global.dominio'))
        ->first();
        ...
        if ($idUsuario) {
          if ($idUsuario->estado_clave === true) {
            ...
            $userDataEmail = DB::connection($name_bdd)
            ->table('public.personas_correo_electronico')
            ->select($campos)
            ->where('id_persona',$idUsuario->id_persona)
            ->first();

            $campos = [ 'primer_nombre', 'primer_apellido'];
            $dataPersona = DB::connection($name_bdd)
              ->table('public.personas')
              ->select($campos)
              ->where('id',$idUsuario->id_persona)->first();

            if ($userDataEmail) {
              $res = DB::connection($name_bdd)
                ->statement("SELECT * FROM actualiza_clave('".$idUsuario->id."', '".bcrypt($pass)."')");
              if ($res) {

                 $estado = DB::connection($name_bdd)
                  ->table('usuarios.usuarios')
                  ->where('id', $idUsuario->id)
                  ->update(['estado_clave' => false]);
          ...
          if ($idUsuario->estado_clave === false) {
            ...
            if ((int) $diferencia->h > 2 || (int)$diferencia->d > 0) {
              if ($idUsuario) {
                ...
                $userDataEmail = DB::connection($name_bdd)
                  ->table('public.personas_correo_electronico')
                  ->select($campos)->where('id_persona',$idUsuario->id_persona)
                  ->first();
      ...
      if ($request->nick === null && $request->correo !== null) {
        ...
        if ($userDataEmail) {
          ...
          $idUsuario = DB::connection($name_bdd)
            ->table('usuarios.usuarios')
            ->select($campos)
            ->where('id_persona',$userDataEmail->id_persona)
            ->first();
          ...
          $dataPersona = DB::connection($name_bdd)
            ->table('public.personas')
            ->select($campos)
            ->where('id',$userDataEmail->id_persona)
            ->first();
            ...
            if ($idUsuario->estado_clave === true) {
              ...
              if ($res) {
                 ...
                 $estado = DB::connection($name_bdd)
                  ->table('usuarios.usuarios')
                  ->where('id', $idUsuario->id)
                  ->update(['estado_clave' => false,'fecha_actualiza' => Carbon::now()->toDateTimeString()]);
                ...
                if ($estado) {
                  ...
                  $data['nombre_comercial'] = $dataPersona->primer_nombre.' '. $dataPersona->primer_apellido;
                  $data['ruc'] = $request->ruc;
                  $data['user_nextbook'] = explode("@", $idUsuario->nick)[0];
                  $data['pass_nextbook'] = $pass;
              ...
              if ((int) $diferencia->h > 2 || (int)$diferencia->d > 0) {
                ...
                if ($res) {
                   $estado = DB::connection($name_bdd)
                    ->table('usuarios.usuarios')
                    ->where('id', $idUsuario->id)
                    ->update(['fecha_actualiza' => Carbon::now()->toDateTimeString()]);
