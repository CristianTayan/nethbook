...
class Administracion_Empresa extends Controller {
  public function __construct(Request $request) {
   ...
   public function getDatosAdicionalesEmpresa(Request $request) {
    $data_empresa=DB::connection($this->name_bdd)
    ->table('administracion.informacion_empresa_tbl')
    ->where('id_sucursal',0)->first();

    if ($data_empresa) {
      $datos_empresa = array(
        'mision' => $data_empresa->mision,
        'vision' => $data_empresa->vision,
        'slogan' => $data_empresa->slogan
      );
    ...
    return response()->json(['respuesta' => null], 200);    
  }

  public function addInformacionEmpresas(Request $request) {
    $x=0;
    $data_sucursal = DB::connection($this->name_bdd)
      ->table('administracion.informacion_empresa_tbl')
      ->where('id_sucursal',$x)
      ->first();

    if ($data_sucursal === null) { 
      $data=DB::connection($this->name_bdd)
      ->table('administracion.informacion_empresa_tbl')
      ->insert([
        'id_empresa'=>1,
        'id_tipo_empresa'=>0,
  ...
  public function Get_Tipo_Bienes_Servicios(Request $request) {
    $resultado = DB::connection($this->name_bdd)
      ->table('administracion.tipo_bienes_servicios')
  ...
  public function Update_Password(Request $request) {
    $resultado = DB::connection($this->name_bdd)
      ->statement("SELECT * FROM actualiza_clave('" . $this->user->sub . "','" . bcrypt($request->pass) . "')");
    if ($resultado){
      return response()->json(["respuesta" => true], 200);
    }
  ...
  public function Update_Informacion_Empresa(Request $request) {
    DB::connection($this->name_bdd)
      ...
      ->where('id', $request->id)
      ->update(['primer_nombre' => $request->primer_nombre, 'segundo_nombre' => $request->segundo_nombre, 'primer_apellido' => $request->primer_apellido, 'segundo_apellido' => $request->segundo_apellido, 'id_localidad' => $request->id_localidad, 'calle' => $request->calle, 'transversal' => $request->transversal, 'numero' => $request->numero]);
    DB::connection($this->name_bdd)
      ->table('public.personas_correo_electronico')
      ->where('correo_electronico', $request->correo_electronico)->update(['correo_electronico' => $request->correo_electronico]);
    DB::connection($this->name_bdd)
      ->table('public.telefonos_personas')
      ->where('numero', $request->numero)
      ->update(['numero' => $request->numero, 'id_operadora_telefonica' => $request->id_operadora_telefonica]);
