...
class Marcas extends Controller
{
    ...
    public function Existencia_Marca(Request $request)
    {
        $existencia=DB::connection($this->name_bdd)->table('inventario.marcas')->where('nombre',$request->nombre)->get();
    ...
    if ($request->has('filter')&&$request->filter!='') {
        ...
        $data=DB::connection($this->name_bdd)->table('inventario.marcas')->where('estado','A')
                                                ->where('nombre','LIKE','%'.$request->input('filter').'%')
                                                //->orwhere('descripcion','LIKE','%'.$request->input('filter').'%')
                                                ->orderBy('nombre','ASC')->get();
    }else{
        $data=DB::connection($this->name_bdd)->table('inventario.marcas')->where('estado','A')->orderBy('nombre','ASC')->get();
    }
    ...
    {
    $data=DB::connection($this->name_bdd)->table('inventario.marcas')->where('id',$request->id)->update(['nombre' => $request->nombre , 'descripcion' => $request->descripcion]);
    return response()->json(['respuesta' => true], 200);
    }

    public function Delete_Marca(Request $request)
    {
    $data=DB::connection($this->name_bdd)->table('inventario.marcas')->where('id',$request->id)->update(['estado'=>'P']);
    return response()->json(['respuesta' => true], 200);
    }
}
